...
#include <sys/time.h>
#include <time.h>
#define NUMERO_MAX_CONEXIONES 2
...
int main(int argc, char** argv){
	...
	int toquesFlota[2][7];
	bool jugador1Listo = false, jugador2Listo = false;
	char queHacer[100];
	char nombre[100];
	int numClientes,i;
	int offset = 0;
	...
	char nombresClientes[NUMERO_MAX_CONEXIONES] [100];
	char idClientes[NUMERO_MAX_CONEXIONES] [100];
	int descriptores[NUMERO_MAX_CONEXIONES];
	for(i = 0; i < NUMERO_MAX_CONEXIONES;i++)
	{
		descriptores[i] = -1;
	...
	else if(pidEsperaClientes == 0)
	{
		while(1)
		{
			...
			timeout.tv_usec = 50;
			timeout.tv_sec = 0;
			if(select(fd_maximo,&paraleer,NULL,NULL,&timeout) > 0)
			{
				...
				if(FD_ISSET(socket1,&paraleer))
				{
					...
					printf("\n");
					if(numClientes < NUMERO_MAX_CONEXIONES)
					{
						numClientes++;
						...
						else
						{
							...
							while(feof(serv) == 0)
							{
								...
								{
									for(int k = 0; k < NUMERO_MAX_CONEXIONES;k++)
									{
